...
class Crossword(object):
    def _init_(self, cols, rows, empty='-', maxloops=2000, available_words=[]):
        self.cols = cols
        self.rows = rows
        self.empty = empty
    ...
    def solution(self):
        solution_grid = []  # Initialize an empty list to store the rows of the grid
        
        # Iterate over each row in the grid
        for r in range(self.rows):
            row = []  # Initialize an empty list to store the values of the current row
            for c in self.grid[r]:
                row.append(c)  # Add each element of the row to the row list
            solution_grid.append(row)  # Add the row to the solution grid
        
        return solution_grid  # Return the 2D list
    ...
    # Order the words if needed
        if order:
            self.order_number_words()
        copy = self
        # Add numbers to the grid at word positions
        for word in self.current_word_list:
            copy.set_cell(word.col, word.row, str(word.number))
        # Initialize a 2D list to represent the grid
        grid_2d = []
        # Populate the grid with cells
        for r in range(copy.rows):
            row = []  # This will represent the row as a list
            for c in copy.grid[r]:
                row.append(c)
            grid_2d.append(row)
        # Replace alphabetic characters with spaces (' ') to remove the words
        # This is where you remove the words and keep the number placements.
        grid_2d = [
            [re.sub(r'[a-zA-Z]', ' ', str(cell)) if isinstance(cell, str) else cell for cell in row]
            for row in grid_2d
        ]
        return grid_2d
    ...
    def legend(self):
        legend_list = []  # Initialize an empty list to store the legend entries
        
        # Iterate over each word in the current_word_list
        for word in self.current_word_list:
            # Format the string for the current word and append to the list
            legend_entry = '%d. (%d,%d) %s: %s' % (word.number, word.col, word.row, word.down_across(), word.clue)
            legend_list.append(legend_entry)
        
        return legend_list  # Return the list of legend entries



class Word(object):
    def _init_(self, word=None, clue=None):
